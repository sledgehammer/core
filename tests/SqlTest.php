...
class SqlTest extends TestCase
{
    ...
    {
        $sql = \Sledgehammer\select('*')->from('customers')->where(['OR', 'bonus = 1', ['AND', 'special = 1', 'age < 12']]);
        $this->assertSame((string) $sql, 'SELECT * FROM customers WHERE bonus = 1 OR (special = 1 AND age < 12)');

        $sql = \Sledgehammer\select('*')->from('customers')->orWhere(['AND', 'bonus = 1', ['AND', 'special = 1', 'age < 12']]);
        $this->assertSame((string) $sql, 'SELECT * FROM customers WHERE bonus = 1 AND special = 1 AND age < 12');

        $sql = \Sledgehammer\select('*')->from('customers')->where(['bonus = 1'])->orWhere('special = 1')->andWhere('age < 12');
        $this->assertSame((string) $sql, 'SELECT * FROM customers WHERE (bonus = 1 OR special = 1) AND age < 12');
        ...
        $sql->where = [
            ...
            [
                ...
                [
                    ...
                    ['OR'], // nodes with only a logical operator are skipped.
                    ['g = 7'], // nodes without logical operator, but containing only 1 condition are treated as a that condition.
                    ['OR', 'h = 8'], // nodes with only 1 condition are treated as a that condition. The swap between AND -> OR doesn't cause ()
                    ['OR', 'i = 9', 'j = 10'],
                ],
            ],
        ];
