<?php
/**
 * Shortcuts to Sledgehammer namespaced functions.
 * Too prevent fatal "Cannot redeclare" errors, the functions are only defined if they aren't already defined by another framework.
 *
 * @package Core
 */
...
if (function_exists('warning') === false) {
	...
	function warning($message, $information = null) {
		Sledgehammer\warning($message, $information);
	}
...
if (function_exists('deprecated') === false) {
	...
	function deprecated($message = 'This functionality will no longer be supported in upcomming releases', $information = null) {
		Sledgehammer\deprecated($message, $information);
	}
