...
class GenerateStaticLibrary extends Util {
	...
	function generateContent() {
		...
		$dialog = new DialogBox('Optimize AutoLoader', $warningMessage, array('cancel' => 'Cancel', 'continue' => array('label' => 'Continue', 'class' => 'btn btn-primary')));
		$answer = $dialog->import($error);
		if ($answer == 'continue') {
			if (!$this->isWritable()) {
				return MessageBox::error('Generating failed', 'Make sure the webuser is allowed to write "'.$this->paths['project'].'AutoLoader.db.php"');
			}
