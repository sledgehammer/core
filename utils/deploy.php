...
define('\Sledgehammer\ENVIRONMENT', 'development');
require_once dirname(__DIR__).'/bootstrap.php';

if (strlen(\Sledgehammer\PATH) <= 4) { // De PATH constante controleren. strlen("PATH") == 4
    error('Invalid PATH');
}
...
shell_execute('chmod a+wrX '.escapeshellarg(\Sledgehammer\PATH.'tmp/'));
passthru('chmod -R a+wrX '.escapeshellarg(\Sledgehammer\PATH.'tmp/')); // bestanden binnen de tmp kunnen al van de webuser zijn. Ookal gaat dit fout, moet dit script verdergaan.
echo "  done.\n\n";
