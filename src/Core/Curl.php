...
//		CURLOPT_TIMEOUT => ?, // Don't allow the request to take more time the the internal php timeout.
     ...
     * @var int
     */
    public static $maxConcurrent = 64;
     ...
     */
    public static function get($url, $options = [], $callback = null)
    {
    ...
    public function isComplete()
    {
        ...
        do {
            ...
            if ($message !== false) {
                ...
                foreach (self::$requests as $index => $curl) {
                    if ($curl->handle === $message['handle']) {
                        unset(self::$requests[$index]); // Cleanup global curl pool
     ...
     */
    public function resend($options = [])
    {
        if ($this->state !== 'ABORTED') {
            $this->abort();
        }
        $index = array_search($this, self::$requests, true);
        if ($index !== false) {
            unset(self::$requests[$index]);
        }
        $this->start($options);
    }
    ...
    {
        $this->state = 'ERROR';
        self::$requests[] = $this; // Watch changes
        // Setting options
     ...
     * @return array
     */
    private static function defaults($options = [])
    {
        $defaults = self::$defaults;
