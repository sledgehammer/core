...
class Library extends Object {
	...
	private
		...
		$defaultSettings = array(
			...
			'eof_check' => true, // Controleer op correct einde van de php bestanden
	...
	function __construct($path) {
		$this->path = $path;
		$this->cachePath = $path.TMP_DIR.'library/';
	}

	/**
	 * De class en interface definities van de applicatie in $this->path inladen.
	 * Als "{$this->path}/library.db.php" bestaat, wordt deze ingeladen, anders worden de php-bestanden ge-inspecteert voor class en interface definities.
	 *
	 * @return void
	 */
	function init() {
		if (!file_exists($this->path.'library.db.php')) {
			$modules = SledgeHammer::getModules();
			$this->extract_definitions_from_modules($modules);
		} else {
			include($this->path.'library.db.php');
			if (!isset($classes) || !isset($interfaces)) {
				notice('$classes of $interfaces definitions not found in "'.$this->path.'library.db.php"');
			}
	...
	function declare_class($class) {
		...
		if (!class_exists($class, false)) {
			error('A library file is corrupt, class "'.$class.'" not found in "'.$class_info['filename'].'"');
		}
		return true;
	}
	...
	function declare_interface($interface) {
		...
		if (!interface_exists($interface)) {
			error('A library file is corrupt, interface "'.$interface.'" not found in "'.$filename.'"');
		}
		return true;
	}
		...
		foreach ($modules as $module_name => $module) {
			...
			if (is_dir($class_folder)) {
				if ($this->enable_cache) {
					...
					if (!mkdirs(dirname($cache_file))) {
						...
						if (file_exists($cache_file)) {
							...
							if (file_exists($class_folder.'/library.ini')) {
								$library_ini = parse_ini_file($class_folder.'/library.ini', true);
								$revalidate_cache_delay = isset($library_ini['revalidate_cache_delay']) ? $library_ini['revalidate_cache_delay'] : $this->defaultSettings['revalidate_cache_delay'];
							} else {
				...
				if ($this->enable_cache && $this->extractErrors == false) { // Zijn er geen fouten opgetreden
					...
					$this->write_cache_file($cache_file, $new_classes, $new_interfaces);
				}
			}
		}
	...
	function extract_definitions_from_folder($folder, $settings = NULL) {
		...
		if (file_exists($folder.'/update_library.ini')) {
			deprecated('"update_library.ini" must be renamed to library.ini');
		}
		if (file_exists($folder.'/library.ini')) {
			$library_ini = parse_ini_file($folder.'/library.ini', true);
			foreach ($library_ini as $class => $info) {
				if (is_array($info)) { // Is het geen library instelling maar een class definitie?
					$info['filename'] = $this->strip_path($this->unix_path($folder.'/'.$info['filename']));
					$this->classes[$class] = $info;
				} elseif (!in_array($class, array_keys($this->defaultSettings))) { 			
					$this->extract_notice('Invalid setting: "'.$class.'" = '.syntax_highlight($info).' in "'.$this->strip_path($folder).'/library.ini"', array('Available settings' => array_keys($this->defaultSettings)));
				}
			}
			$settings = array_merge($settings, $library_ini); // De settings aanpassen aan wat in de library.ini staat
		...
		foreach ($DirectoryIterator as $Entry) {
			if ($Entry->isDir()) {
				...
				if ($Entry->getFilename() != 'library.ini') {
					if ($settings['php_extension_check']) {
			...
			} elseif ($definitions = $this->extract_definitions_from_file($Entry->getPathname(), $settings)) {
	...
	/**
	 * Schrijf een cache bestand voor de Library class in de PATH map.
	 ...
	 */
	function write_library_db() {
		return $this->write_cache_file($this->path.'library.db.php', $this->classes, $this->interfaces);
	}

	/**
	 * Een cache bestand 
	 */
	private function write_cache_file($filename, $classes, $interfaces) {
		if($fp = fopen($filename, 'w')) {
			fputs($fp, "<?php\n/**\n * Generated library-definitions-cache\n */\n\$classes = array(\n");
			// classes
			ksort($classes);
	...
	private function extract_definitions_from_file($filename, $settings) {
		...
		if ($settings['eof_check'] && substr($source, -3, -1) != '?>' && substr($source, -2) != '?>') {
			$this->extract_notice('Invalid end of file: "'.$this->strip_path($filename).'", expecting "?>"');
		}
		if ($settings['bof_check'] && substr($source, 0, 12) != "<?php\n/**\n *" && substr($source, 0, 14) != "<?php\r\n/**\r\n *") { 
			$this->extract_notice('Invalid start of file:  "'.$this->strip_path($filename).'"', array('Expecting' => "\"\n<?php\n/**\n *\""));
		}
		...
		foreach ($tokens as $token) {
			if (empty($statusses_definitions[$index])) {
				$statusses_definitions[$index] = array(
					...
					'interface' => false,
					'info' => array(
						'filename' => $this->strip_path($this->unix_path($filename)),
					),
				);
		...
		foreach ($statusses_definitions as $definition_state) {
			if ($definition_state['interface'] == 'DEFINED') {
				...
				if ($settings['matching_filename'] && substr(basename($filename), 0, -4) != $definition_state['info']['class']) {
					$this->extract_notice('Filename doesn\'t match classname "'.$definition_state['info']['class'].'" in "'.$this->strip_path($filename).'"');
				}
	...
	/**
	 * Zet de directory-separators om naar '/'
	 * Omdat op windows "C:/path/to/" wordt goed gekeurd en bij "C:\new folder" fouten geeft
	 * @todo: Bob: Nettere oplossing bedenken voor windows filesystem problemen
	 */
	private function unix_path($path) {
	...
	private function unexpected_token($token, $filename) {
		...
		$this->extract_notice('Unexpected token: '.$error.' in "'.$this->strip_path($filename).'"');
	}
	 ...
	 */
	private function strip_path($filename) {	
	...
	private function fullpath($filename) {
		...
		} elseif (preg_match('/^[a-Z]{1}:/', $filename)) { //  Een windows path begint met bv 'C:\'
			return $filename; // Absoluut path
		} 
