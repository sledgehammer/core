...
class AutoLoader extends Object {
	...
	function __construct($path) {
		$this->path = $path;
		$this->cachePath = $path.'/tmp/library/';
	}
	...
	function extract_definitions_from_folder($folder, $settings = NULL) {
		...
		if (file_exists($folder.'/library.ini')) {
			deprecated('"library.ini" must be renamed to autoloader.ini');
		}
		if (file_exists($folder.'/autoloader.ini')) {
			...
			foreach ($autoloader_ini as $class => $info) {
				if (is_array($info)) { // Is het geen autoloader instelling maar een class definitie?
					...
					$this->extract_notice('Invalid setting: "'.$class.'" = '.syntax_highlight($info).' in "'.$this->strip_path($folder).'/autoloader.ini"', array('Available settings' => array_keys($this->defaultSettings)));
				}
			}
		...
		foreach ($DirectoryIterator as $Entry) {
			if ($Entry->isDir()) {
				...
				if ($Entry->getFilename() != 'autoloader.ini') {
	...
	private function fullpath($filename) {
		...
		} elseif (preg_match('/^[a-z]{1}:/i', $filename)) { //  Een windows path begint met driveletter. bv 'C:\'
			return $filename; // Absoluut path
		} 
