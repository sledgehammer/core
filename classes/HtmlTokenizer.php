 ...
 * @link http://www.whatwg.org/specs/web-apps/current-work/multipage/syntax.html
 * @package Core
 */
class HtmlTokenizer extends Object implements \Iterator {
    ...
    function __construct($html, $initialState = 'CONTENT') {
        $this->html = $html;
    ...
    function parseTagBody() {
        ...
        if ($pos === $this->position) { // Invalid start character.
            if ($this->getChar() === '/') { 
                $this->warning('Unexpected "/" inside a tag');
                $this->buildToken($this->position, 'T_INVALID');
                return;
            }
            // Use "=" for the attribute name.
            $pos = $this->firstOccurrance(array_merge($this->wsArray, array('=', '>', '/')), $match, 1);
        }
        ...
        $this->buildToken($this->position, 'T_ASSIGNMENT'); // assignment operator
        $this->collectWhitespace();
        $this->state = 'VALUE';
    }
    ...
    function buildToken($endPosition, $tokenType = null) {
        ...
        if ($tokenType === null) {
            ...
            $this->tokenQueue[] = array($tokenType, $value, $this->position);
            switch ($tokenType) {
                case 'T_ATTRIBUTE':
                    if (preg_match('/["\'`=<]/', $value, $match)) {
                        $this->warning('Invalid character: "'.$match[0].'" in attribute "'.$value.'"');
                    }
                    break;
            }
        }
     ...
     * relatief vanaf de huidige $position
     */
    function strpos($needle, $offset = 0) {
        return strpos($this->html, $needle, $this->position + $offset);
    }
     ...
     * relatief vanaf de huidige $position
     */
    function stripos($needle, $offset) {
        return stripos($this->html, $needle, $this->position + $offset);
    }

    /**
     * @return int de positie van de needle die het eerste voorkomt in de string
     */
    function firstOccurrance($needles, &$match = null, $offset = 0) {
        $pos = false;
        foreach ($needles as $needle) {
            $needlePos = $this->strpos($needle, $offset);
            if ($needlePos === false) {
                continue;
            }
            if ($pos === false) {
